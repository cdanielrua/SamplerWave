...
#include "hardware/clocks.h" // Necesario para init_audio
...
const uint32_t DEBOUNCE_DELAY_US = 150000; // Retardo de 150ms
...
void update_leds() {
    for (int i = 0; i < NUM_PIXELS; ++i) {
        if (i == current_step) {
            ...
            for(int j = 0; j < NUM_SAMPLES; ++j) if(sequence_grid[j][i]) { active_step = true; break; }
        ...
        if (voices[i].pos < voices[i].len) {
            for (int j = 0; j < sizeof(dma_buffer); ++j) {
                if (voices[i].pos < voices[i].len) {
                    ...
                    if (mixed_sample > 255) mixed_sample = 255; else if (mixed_sample < 0) mixed_sample = 0;
    ...
    for (int i = 0; i < NUM_SAMPLES; ++i) {
        if (gpio == BUTTON_PINS[i]) {
            ...
            if (diff_us > DEBOUNCE_DELAY_US) {
                ...
                play_sample(i); // <-- Ahora reproducimos el sonido
...
bool sequencer_timer_callback(struct repeating_timer *t) {
    ...
    for (int i = 0; i < NUM_SAMPLES; ++i) {
        if (sequence_grid[i][current_step]) {
            play_sample(i); // <-- También reproducimos los sonidos de la secuencia
        }
    }
    step_changed = true;
    return true;
...
int main() {
    ...
    init_audio(); // <--- Reactivamos la inicialización de audio
    init_buttons();
    
    for (int i = 0; i < NUM_SAMPLES; ++i) { last_press_time[i] = nil_time; }

    struct repeating_timer timer;
    add_repeating_timer_ms(-125, sequencer_timer_callback, NULL, &timer);
